...
// Paramètres :
//    POST reference : référence de l'article que l'on veut afficher
...
// reference
// debug : print_r($_POST);
// Si on a  envoyé reference, on le récupére
if (isset($_POST["reference"])) {
	$reference = $_POST["reference"];
} else {
	// On n'a pas envoyé de référence
	echo "Vous devez passer par le formulaire et fournir une référence";
	exit;
}

// Vérifier si la référence existe : on a un article dans $articles dont l'idex est la référence choisie
if ( ! isset($articles[$reference])) {
	echo "L'article $reference n'existe pas";
	exit;
}
